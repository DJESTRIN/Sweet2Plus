...
import numpy as np
try:
    # Only needed by this module's own gather_data()/__main__ CLI path, which pulls in a heavy
    # suite2p/torch/optuna import chain. Importing it lazily/optionally means this module (and its
    # GLM/statistics functionality) can still be imported and used -- including inside joblib
    # subprocess workers, which re-import the module fresh -- in environments without those
    # unrelated raw-imaging dependencies installed.
...
class circuit_regression:
    ...
    def run_glm(self, coef_file='allcoefs.csv', model_file='allmodels.csv', n_jobs=-1):
...
def gather_data(parent_data_directory,drop_directory,file_indicator='obj'):
    ...
    if LoadObj is None:
        raise ImportError(
            "LoadObj is unavailable because NeuroSweet.core.SaveLoadObjs (and its suite2p/torch/optuna "
            "dependency chain) could not be imported. Install those dependencies to use gather_data(), or "
            "use a lightweight equivalent loader instead."
        )
