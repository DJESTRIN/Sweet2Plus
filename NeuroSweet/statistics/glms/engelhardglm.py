...
Date: 03-05-2026
"""
# import NeuroSweet code
try:
    # Only needed by this module's own proc()/__main__ CLI path, which pulls in a heavy
    # suite2p/torch/optuna import chain via NeuroSweet.core.SaveLoadObjs -> NeuroSweet.core.core.
    # Importing it lazily/optionally means this module (and its GLM fitting functionality) can still
    # be imported and used -- including inside joblib subprocess workers, which re-import the module
    # fresh -- in environments without those unrelated raw-imaging dependencies installed.
    from NeuroSweet.core.SaveLoadObjs import gather_data  # Custom function which basically loads in all of the activity, event, and info data
except ImportError:
    gather_data = None
...
class engelhardglm(object):
    def __init__(self, activity, timestamps, info, dropdir, post_event_frames = 20, model_type = 'glm', selected_family_glm = None, 
        ...
        # convention used by circuit_regression (NeuroSweet/statistics/circuit_coefficient_clustering.py),
    ...
    def fit(self, n_jobs=-1):
        ...
        Parallel(n_jobs=n_jobs, backend="loky", verbose=10)(delayed(process_single_neuron)
...
def proc():
    ...
    if not data_provided:
        if gather_data is None:
            raise ImportError(
                "gather_data is unavailable because NeuroSweet.core.SaveLoadObjs (and its suite2p/torch/"
                "optuna dependency chain) could not be imported. Install those dependencies to use this "
                "CLI path, or use a lightweight equivalent loader instead."
            )
