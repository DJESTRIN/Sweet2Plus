...
"""
from NeuroSweet.core.core import pipeline, corralative_activity 
from NeuroSweet.core.behavior import load_serial_output
import numpy as np
import warnings
import tqdm
import pickle
from NeuroSweet.core.SaveLoadObjs import SaveObj, LoadObj, SaveList, OpenList
...
def delete_2p_obj_files(input_directory):
    ...
    response = input(f"Are you sure you want to delete all NeuroSweet objects in '{input_directory}'? (yes/no): ").strip().lower()
    if response in {'yes', 'y'}:
        ...
        print(f'You have decided NOT to delete all NeuroSweet files in {input_directory}')
