...
# Import dependencies
import argparse
from NeuroSweet.core.SaveLoadObjs import LoadObj
from NeuroSweet.weight_modeling.NetworkArchitectures import SharedTransformerNN, weighted_mse_loss
import torch
...
def zscore_data_from_obj(obj_file_oh):
    """ Load in ztrace data from NeuroSweet obj file """
...
class Education():
    ...
    def __init__(self, data, model=[], num_data_points=300, epochs=100, print_training=True,
                 ...
                 default_layers = 3, default_dropout = 0.1, drop_directory=r'C:\Users\listo\NeuroSweet\my_figs',drop_filename='filename'):
    ...
    def hypertuning_study(self):
        ...
        def objective(trial):
            ...
            X_train, y_train, X_val, y_val, X_test, y_test = self.get_data(num_data_points=num_data_points)
            train_loader, val_loader, test_loader = self.convert_to_torch_loader(X_train, y_train, X_val, y_val, X_test, y_test)
...
def synthetic_data(model, torch_data_loader, pseudo_time_points=4500):
    ...
    """
    np.random.seed(2256)
    random_index = random.randint(0,len(torch_data_loader)-1)
    ...
    for idx in range(pseudo_time_points):
        ...
        # Update activity series data: drop oldest timestep and append the new prediction
        activity_series = activity_series[1:,:]
        activity_series = np.concatenate([activity_series, output.reshape(1, -1)], axis=0)
...
if __name__=='__main__':
    ...
    # objoh = capture_model_weights(torch_file=r'C:\Users\listo\NeuroSweet\my_figs\weight_modeling_test_output\solo_model.pth',
    #                       drop_directory=r'C:\Users\listo\NeuroSweet\my_figs\weight_modeling_test_output', 
